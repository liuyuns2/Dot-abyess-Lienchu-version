...
| `提取劇情文本會用到的/` | **劇情文本的抓取工具鏈**（已收進 repo，見下方「兩條產線」） |
...
│  工具：提取劇情文本會用到的/tools/OfficialNovelUpdate.ps1│
│  手冊：提取劇情文本會用到的/官方更新交接手冊.md          │
...
A 線的細節（環境安裝、一鍵腳本、驗證、L2D 差異比對）全在 `提取劇情文本會用到的/` 裡（先讀該資料夾的 `README.md`），本檔不重複。只提三個銜接重點：
...
- **先搞清楚自己在做哪條產線**（見開頭「兩條產線」）。劇情文本走 `提取劇情文本會用到的/tools/`，UI／系統文字走本 repo 根的 `tools/`。兩條都做完才算一次完整更新。
