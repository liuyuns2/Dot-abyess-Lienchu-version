...
> 🩸 **2026-08-10 實際事故。** 兩支工具原本都把
> `m_character_abilities` / `m_character_action_skills` 整張表排除掉——政策禁的
> 其實只有 `name` 一欄。結果 `m_character_action_skills/description`（角色詳情與
> 抽卡畫面上的主動技能說明）從來沒被抽出過，每個新角色上線都是一片日文。
> 同一時間 `m_gacha_groups/description`（轉蛋詳細說明，開頭就是角色名＋技能介紹）
> 因為既有譯文全被前手誤填在 `name` 欄，這一欄永遠不算「已知」，同樣整欄被跳過。
> 兩個盲區疊在一起，【水着】ホノカ 上線時抽卡畫面的名稱與技能介紹全是日文。
> 現在禁翻改成欄位層級、扁平檔（`ui_texts`/`names`）也納入覆蓋判斷、
> 未知欄位會另外出清單。
...
```
m_character_abilities/name        ← 這一欄清空，不翻
m_character_action_skills/name    ← 這一欄清空，不翻
m_gacha_group_movies/skill_name   ← 這一欄清空，不翻
```

新版本冒出的能力名/技能名**一律不翻**。補完後要驗證這三個**欄位**仍為空。

⚠️ **禁的是欄位，不是整張表。** 同表的 `m_character_action_skills/description`
（角色詳情／抽卡畫面的主動技能說明）是**要翻的**，2026-08-10 就是因為兩支工具
把整張表排除掉而整批漏譯，細節見 1-4。

走 1-6 的 `merge_translated.py` 會自動擋掉這三個欄位，但**手動編輯 `static/zh_Hant.json` 時沒有
