...
| `E:\離線板\files\官方更新交接手冊.md` | **劇情文本的抓取流程**（見下方「兩條產線」） |
...
│  工具：E:\離線板\files\OfficialNovelUpdate.ps1（一鍵）  │
│  手冊：E:\離線板\files\官方更新交接手冊.md              │
...
A 線的細節（環境安裝、一鍵腳本、驗證、L2D 差異比對）全在那份手冊裡，本檔不重複。只提三個銜接重點：
...
- **先搞清楚自己在做哪條產線**（見開頭「兩條產線」）。劇情文本走 `E:\離線板\files` 的工具鏈，UI／系統文字走本 repo 的 `tools/`。兩條都做完才算一次完整更新。
