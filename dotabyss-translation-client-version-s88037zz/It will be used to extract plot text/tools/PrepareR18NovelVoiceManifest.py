...
def is_r18_novel_voice(primary_key: str) -> bool:
    lowered = primary_key.lower().replace("\\", "/")
    return (
        lowered.endswith(".bundle")
        and "r18-only" in lowered
        and "workunit_novel_r18_voice" in lowered
        and ".acb_" in lowered
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Create a manifest for DotAbyss r18 novel voice ACB bundles.")
    ...
    for asset in data.get("assets") or []:
        ...
        if not is_r18_novel_voice(primary_key):
            continue
    ...
    print(f"R18 novel voice ACB bundles: {len(rows)}")
