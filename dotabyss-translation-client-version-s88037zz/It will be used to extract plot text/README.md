...
## ⚠️ 使用限制（2026-09-21 新增，請先讀完）

**這套工具鏈只有一個用途：抽取官方劇情原文，翻成繁體中文回饋給玩家。**

2026-09-21 起官方在 API 前面掛了 WAF，會**認 TLS 指紋擋掉腳本流量**（症狀見
`官方更新交接手冊.md` §8）。`tools/DotAbyss.py` 裡的 `TlsAdapter` 就是為了讓抽取
流程在改版後還跑得動而繞過那道偵測的。這段程式碼有明確邊界：

**不得**用於——

- 工作室、代練、刷帳號、自動化遊玩、機器人農場
- 任何形式的商業營利或轉售
- 高頻／大量請求官方伺服器
- 繞過付費、轉蛋、體力或任何遊戲內限制

官方那道 WAF **本來就是為了擋工作室**而設的。這裡繞過它，是為了在改版後仍能取得
劇情原文來翻譯：一次官方改版只跑一輪、純讀取、不碰任何遊戲內行為。挪作上述用途，
等於坐實官方要擋的那件事，也背離這個專案存在的理由。

請求量請維持在「一次改版跑一輪」的量級：不要反覆重跑、不要調高 `DotAbyss.py` 的
`MAX_THREADS`（預設 16）去壓官方。
...
| `tools/check_update.py` | **跑之前先問這個**：官方更新上線了沒。解官方 `/version` 端點，比對資產版本／masterdata 版本／APK 版本與上次探測的差異 |
| `tools/wait_for_update.sh` | 同上，但輪詢到變動為止（等改版當天用）|
